...
- **Ingestion text hygiene** — feed HTML is stripped from descriptions at ingestion
  ...
  (`LinkNormalizer`).
- **Site-chrome scrub rules are operator data** — the patterns removed from extracted article
  text (login nudges, share bars, "follow us" plugs) are no longer a hard-coded list in
  `ArticleFetcher` but a YAML file named by `fetcher.scrubRulesFile` (`rules: [{name, regex,
  host?, dotAll?}]`; host-scoped rules apply to that host and its subdomains only; see
  `scrub-rules.example.yaml`). Unset = nothing is scrubbed. Invalid regexes and duplicate names
  fail at load with the rule name.
